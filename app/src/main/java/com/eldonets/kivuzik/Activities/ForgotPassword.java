...
import com.eldonets.kivuzik.Utilities.Dialog_Show;
    ...
    public void onClick(View v)
    {
        if (v == btn_send_mail)
        {
            if (edttxt_email_forgot.getText().toString().trim().length() == 0) {
                Utility.showAlert(ForgotPassword.this, "Alert","Veillez entrez votre email", null, "Ok");
            }
            else
            {
                ...
                {
                    edttxt_email_forgot.setError("Email invalid");
                }
               else
                {
                GetData();
