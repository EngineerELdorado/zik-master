...
if(isset($_REQUEST['tracks']) && $_REQUEST['tracks']=='true') :
     mysqli_set_charset( $conn, 'utf8');  
       $query = $conn->query("select tracks.id,tracks.title,tracks.description,tracks.uid,users.first_name,users.last_name,tracks.name,tracks.tag,tracks.art,tracks.buy,tracks.record,tracks.release,tracks.license,tracks.size,tracks.download,tracks.time,tracks.public,tracks.likes,tracks.downloads,tracks.views from tracks join users on users.idu = tracks.uid where tracks.public = '1' ORDER BY tracks.id DESC");
       $num = $query->num_rows;
